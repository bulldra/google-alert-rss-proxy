...
```mermaid
flowchart TD
    A[入力エントリ群] --> S1[Stage 1: URLブラックリスト]
    S1 -->|除外: TLD / ドメイン / パターン| E1[Early Exit: url_blacklist]
    S1 -->|通過| S2[Stage 2: タイトルブラックリスト]
    
    S2 -->|除外: NGキーワード| E2[Early Exit: title_blacklist]
    S2 -->|通過| S3[Stage 3: 重複判定 (過去7日分GCSキャッシュ含む)]
    
    S3 -->|除外: URL重複 / レーベンシュタイン比 > 0.7| E3[Early Exit: duplicate]
    S3 -->|通過 (候補エントリ群)| S4[Stage 4: ジャンルブラックリスト (AI Slop & 求人判定)]
    
    subgraph Jev System One API
        S4 -.-> J[Jev System One 評価<br/>is_ai_slop, job_posting, feed_category, feed_priority]
    end
    
    S4 -->|除外: AI Slop / 求人情報 / サイト案内 / 薄い内容| E4[Early Exit: genre_blacklist]
    S4 -->|通過 (採用エントリ)| OUT[最終出力フィード & GCS保存]
```
